...
const formSchema = z.object({
  name: z.string().min(1, "Le produit doit contenir au moins 1 caractères."),
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    console.log(data.images); // Vérifie que toutes les images sont présentes ici
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(
      ...
      <Form {...form}>
        ...
        >
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Les Images</FormLabel>
                <FormControl>
                  <ImageUpload
                    value={field.value.map((image) => image.url)} // On passe un tableau d'images
                    disabled={loading}
                    onChange={
                      (url) => field.onChange([...field.value, { url }]) // Ajout de chaque nouvelle image
                    }
                    onRemove={(url) =>
                      field.onChange([
